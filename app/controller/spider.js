...
class SpiderController extends Controller {
  async scrapeData() {
    ...
    try {
      ...
      while (currentScrollHeight < maxScrollHeight && scrollTimes < maxScrollTimes) {
      ...
      attrItems.each((index, element) => {
        ...
        if (itemNameElement.length && itemNameElement.text().trim() === '产品名称') {
      ...
      tableList.each((index, element) => {
        ...
        if (styleAttributeValue) {
          ...
          backgroundURLMatch = styleAttributeValue.match(/url\(["']?([^"']+)["']?\)/);
        }
        ...
        specTable.push({
          name0: skuItemName,
          goods_picture: backgroundURLMatch?.length > 0 ? backgroundURLMatch[1] : manImageList[4],
